...
Contributions: M. Tarenskeen, N. Liberg, Paul Villiger, Janus Meuris, Larry Myer
scough,
...
without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR
 PURPOSE.
See the Lesser GNU General Public License for more details. <http://www.gnu.org/
licenses/lgpl.html>.
'''
...
note_ornamentation_map = {        # for notations/, modified from EasyABC
    ...
    'articulations/staccatissimo':'!wedge!', # not sure whether this is the righ
t translation
    'articulations/spiccato':     '!wedge!', # not sure whether this is the righ
t translation
    'articulations/breath-mark':  '!breath!', # this may need to be tested to ma
ke sure it appears on the right side of the note
...
percSvg = '''%%beginsvg
    ...
    <text id="normal-" x="-3.7" y="0">&#xe0a3;</text>
    <text id="normal+" x="-3.7" y="0">&#xe0a4;</text>
    <g id="circle-x"><text x="-3" y="0">&#xe263;</text><circle r="4" class="stro
ke"></circle></g>
    <g id="circle-x-"><text x="-3" y="0">&#xe263;</text><circle r="4" class="str
oke"></circle></g>
    <path id="triangle" d="m-4 -3.2l4 6.4 4 -6.4z" class="stroke" style="stroke-
width:1.4"></path>
    <path id="triangle-" d="m-4 -3.2l4 6.4 4 -6.4z" class="stroke" style="stroke
-width:1.4"></path>
    <path id="triangle+" d="m-4 -3.2l4 6.4 4 -6.4z" class="stroke" style="fill:#
000"></path>
    <path id="square" d="m-3.5 3l0 -6.2 7.2 0 0 6.2z" class="stroke" style="stro
ke-width:1.4"></path>
    <path id="square-" d="m-3.5 3l0 -6.2 7.2 0 0 6.2z" class="stroke" style="str
oke-width:1.4"></path>
    <path id="square+" d="m-3.5 3l0 -6.2 7.2 0 0 6.2z" class="stroke" style="fil
l:#000"></path>
    <path id="diamond" d="m0 -3l4.2 3.2 -4.2 3.2 -4.2 -3.2z" class="stroke" styl
e="stroke-width:1.4"></path>
    <path id="diamond-" d="m0 -3l4.2 3.2 -4.2 3.2 -4.2 -3.2z" class="stroke" sty
le="stroke-width:1.4"></path>
    <path id="diamond+" d="m0 -3l4.2 3.2 -4.2 3.2 -4.2 -3.2z" class="stroke" sty
le="fill:#000"></path>
    </defs>
    %%endsvg'''
...
class Measure:
    ...
    def reset (s):      # reset each measure
        ...
        s.lline = ''    # left barline, but only holds ':' at start of repeat, o
therwise empty
...
class Music:
    def __init__(s, options):
        ...
        s.gLyrics = []          # [{num: (abc_lyric_string, melis)},.. for all m
easures in a part]
        s.vnums = {}            # all used voice id's in a part (xml voice id's
== numbers)
        ...
        s.bpl = options.b       # the max number of bars per line when writing a
bc
    ...
    def initVoices (s, newPart=0):
        ...
        for v in s.vnums:
            s.vtimes [v] = 0    # {voice: the end time of the last item in each
voice}
    ...
    def insertElem (s, v, elem):    # insert at the start of voice v in the curr
ent measure
        obj = Elem (elem)
    ...
    def appendObj (s, v, obj, dur):
        ...
        s.incTime (dur)
        if s.tijd > s.vtimes[v]: s.vtimes[v] = s.tijd   # don't update for inser
ted earlier items

    def appendElem (s, v, elem, tel=0):
        s.appendObj (v, Elem (elem), 0)
        if tel: s.cnt.inc ('note', v)   # count number of certain elements in ea
ch voice (in addition to notes)
    ...
    def appendNote (s, v, note, noot):
        ...
        s.lastnote = note           # remember last note/rest for later modifica
tions (chord, grace)
    ...
    def getLastRec (s, voice):
        if s.gMaten: return s.gMaten[-1][voice][-1] # the last record in the las
t measure
        return None                                 # no previous records in the
 first measure

    def getLastMelis (s, voice, num):   # get melisma of last measure
        if s.gLyrics:
            lyrdict = s.gLyrics[-1][voice]  # the previous lyrics dict in this v
oice
            if num in lyrdict: return lyrdict[num][1]   # lyrdict = num -> (lyri
c string, melisma)
        return 0 # no previous lyrics in voice or line number

    def addChord (s, note, noot):   # careful: we assume that chord notes follow
 immediately
    ...
    def addBar (s, lbrk, m): # linebreak, measure data
        if m.mdur and s.maxtime > m.mdur: info ('measure %d in part %d longer th
an metre' % (m.ixm+1, m.ixp+1))
        s.tijd = s.maxtime              # the time of the bar lines inserted her
e
        for v in s.vnums:
            if m.lline or m.lnum:       # if left barline or left volta number
                p = s.getLastRec (v)    # get the previous barline record
                if p:                   # in measure 1 no previous measure is av
ailable
                    x = p.str           # p.str is the ABC barline string
                    if m.lline:         # append begin of repeat, m.lline == ':'
                        x = (x + m.lline).replace (':|:','::').replace ('||','|'
)
                    if s.nvlt == 3:     # add volta number only to lowest voice
in part 0
                        if m.ixp + v == min (s.vnums): x += m.lnum
                    elif m.lnum:        # new behaviour with I:repbra 0
                        x += m.lnum     # add volta number(s) or text to all voi
ces
                        s.repbra = 1    # signal occurrence of a volta
                    p.str = x           # modify previous right barline
                elif m.lline:           # begin of new part and left repeat bar
is required
                    s.insertElem (v, '|:')
            if lbrk:
                p = s.getLastRec (v)    # get the previous barline record
                if p: p.str += lbrk     # insert linebreak char after the barlin
es+volta
            if m.attr:                  # insert signatures at front of buffer
                s.insertElem (v, '%s' % m.attr)
            s.appendElem (v, ' %s' % m.rline)   # insert current barline record
at time maxtime
            s.voices[v] = sortMeasure (s.voices[v], m)  # make all times consist
ent
            lyrs = s.lyrics[v]          # [{number: sylabe}, .. for all notes]
            lyrdict = {}                # {number: (abc_lyric_string, melis)} fo
r this voice
            nums = [num for d in lyrs for num in d.keys ()] # the lyrics numbers
 in this measure
            maxNums = max (nums + [0])  # the highest lyrics number in this meas
ure
            for i in range (maxNums, 0, -1):
                xs = [syldict.get (i, '') for syldict in lyrs]  # collect the sy
llabi with number i
                melis = s.getLastMelis (v, i)  # get melisma from last measure
                lyrdict [i] = abcLyr (xs, melis)
            s.lyrics[v] = lyrdict       # {number: (abc_lyric_string, melis)} fo
r this measure
            mkBroken (s.voices[v])
    ...
    def outVoices (s, divs, ip, isSib): # output all voices of part ip
        vvmap = {}                  # xml voice number -> abc voice number (one
part)
        ...
        for iv in vnum_keys:
            if s.cnt.getv ('note', iv) == 0:    # no real notes counted in this
voice
                continue            # skip empty voices
            if abcOut.denL: unitL = abcOut.denL # take the unit length from the
-d option
            else:           unitL = compUnitLength (iv, s.gMaten, divs) # comput
e the best unit length for this voice
            abcOut.cmpL.append (unitL)  # remember for header output
            vn, vl = [], {}         # for voice iv: collect all notes to vn and
all lyric lines to vl
            for im in range (len (s.gMaten)):
                ...
                for n, (lyrstr, melis) in s.gLyrics [im][iv].items ():
                    if n in vl:
                        while len (vl[n]) < im: vl[n].append ('') # fill in skip
ped measures
                        vl[n].append (lyrstr)
                    else:
                        vl[n] = im * [''] + [lyrstr]    # must skip im measures
            for n, lyrs in vl.items (): # fill up possibly empty lyric measures
at the end
                mis = len (vn) - len (lyrs)
                lyrs += mis * ['']
            abcOut.add ('V:%d' % s.vceCnt)
            if s.repbra:
                if s.nvlt == 1 and s.vceCnt > 1: abcOut.add ('I:repbra 0')  # on
ly volta on first voice
                if s.nvlt == 2 and iv > lvc:     abcOut.add ('I:repbra 0')  # on
ly volta on first voice of each part
            if   s.cpl > 0:  s.bpl = 0      # option -n (max chars per line) ove
rrules -b (max bars per line)
    ...
    # line full (number of bars)
                ...
                lyrlines = sorted (vl.items ())     # order the numbered lyric l
ines for output
...
part
        s.gLyrics = []
        s.cnt.prcnt (ip+1)          # print summary of skipped items in this par
t
        return vvmap

class ABCoutput:
    pagekeys = 'scale,pageheight,pagewidth,leftmargin,rightmargin,topmargin,botm
argin'.split (',')
    def __init__ (s, fnmext, pad, X, options):
        ...
        s.denL = options.d      # denominator of the unit length (L:) from -d op
tion
        s.volpan = int (options.m)  # 0 -> no %%MIDI, 1 -> only program, 2 -> al
l %%MIDI
    ...
    def mkHeader (s, stfmap, partlist, midimap, vmpdct, koppen): # stfmap = [par
ts], part = [staves], stave = [voices]
        accVce, accStf, staffs = [], [], stfmap[:]  # staffs is consumed
        for x in partlist:              # collect partnames into accVce and staf
f groups into accStf
        ...
        for vnum, clef in s.clefs.items ():
            ...
            dmap = midimap [vnum - 1][4:]   # map of abc percussion notes to mid
i notes
            ...
            if s.volpan > 1:    # option -m 2 -> output all recognized midi comm
ands when needed and present in xml
...
lume == 0 is possible ...
            ...
            elif s.volpan > 0: # default -> only output midi program command whe
n present in xml
                if dmap and ch > 0: hd.append ('%%%%MIDI channel %d\n' % ch) # a
lso channel if percussion part
                if prg > 0:  hd.append ('%%%%MIDI program %d\n' % (prg - 1))
...
def abcLyr (xs, melis): # Convert list xs to abc lyrics.
    ...
    res = []
    for x in xs:        # xs has for every note a lyrics syllabe or an empty str
ing
...
def abcdur (nx, divs, uL):      # convert an musicXML duration d to abc units wi
th L:1/uL
    ...
    if den > 64:                # limit the denominator to a maximum of 64
        x = float (num) / den; n = math.floor (x);  # when just above an integer
 n
        if x - n < 0.1 * x: num, den = n, 1;        # round to n
        num64 = 64. * num / den + 1.0e-15           # to get Python2 behaviour o
f round
...
def staffStep (ptc, o, clef, tstep):
    ndif = 0
    if 'stafflines=1' in clef: ndif += 4                    # meaning of one lin
e: E (xml) -> B (abc)
    if not tstep and clef.startswith ('bass'): ndif += 12   # transpose bass ->
treble (C3 -> A4)
...
def setKey (fifths, mode):
    sharpness = ['Fb', 'Cb','Gb','Db','Ab','Eb','Bb','F','C','G','D','A', 'E', '
B', 'F#','C#','G#','D#','A#','E#','B#']
    offTab = {'maj':8, 'ion':8, 'm':11, 'min':11, 'aeo':11, 'mix':9, 'dor':10, '
phr':12, 'lyd':7, 'loc':13, 'non':8}
...
def outVoice (measure, divs, im, ip, unitL):    # note/elem objects of one measu
re in one voice
    ix = 0
    while ix < len (measure):   # set all (nested) tuplet annotations
        nx = measure [ix]
        if isinstance (nx, Note) and nx.fact and not nx.grace:
            ix, tupcnt = insTup (ix, measure, (1, 1))   # read one tuplet, inser
t annotation(s)
        ix += 1
    vs = []
    for nx in measure:
        if isinstance (nx, Note):
            durstr = abcdur (nx, divs, unitL)           # xml -> abc duration st
ring
            chord = len (nx.ns) > 1
...
def sortMeasure (voice, m):
    ...
    for i, nx in enumerate (voice):    # establish sequentiality
        ...
        if isinstance (nx, Elem):
            if nx.tijd < time: nx.tijd = time # shift elems without duration to
where they fit
            v.append (nx)
            time = nx.tijd
            continue
        if nx.tijd < time:                  # overlapping element
            ...
            if v[-1].tijd <= nx.tijd:       # we can do something
                if v[-1].ns[0] == 'z':      # shorten rest
                    ...
                    info ('overlap in part %d, measure %d: rest shortened' % (m.
ixp+1, m.ixm+1))
                else:                       # make a chord of overlap
                    v[-1].ns += nx.ns
                    info ('overlap in part %d, measure %d: added chord' % (m.ixp
+1, m.ixm+1))
                    nx.dur = (nx.tijd + nx.dur) - time  # the remains
                    if nx.dur <= 0: continue            # nothing left
                    nx.tijd = time          # append remains
            else:                           # give up
                info ('overlapping notes in one voice! part %d, measure %d, note
 %s discarded' % (m.ixp+1, m.ixm+1, isinstance (nx, Note) and nx.ns or nx.str))
                continue
        v.append (nx)
        if isinstance (nx, Note):
            if nx.ns [0] in 'zx':
        ...
        time = nx.tijd + nx.dur
    #   when a measure contains no elements and no forwards -> no incTime -> s.m
axtime = 0 -> right barline
    #   is inserted at time == 0 (in addbar) and is only element in the voice wh
en sortMeasure is called
    if time == 0: info ('empty measure in part %d, measure %d, it should contain
 at least a rest to advance the time!' % (m.ixp+1, m.ixm+1))
    return v
        ...
        if x.tag ==  'part-group':
            ...
            if type == 'start':
                if num in e:    # missing stop: insert one
                    xs.append (E.Element ('part-group', number = num, type = 'st
op'))
                    xs.append (x)
                else:           # normal start
                    xs.append (x)
...
def parseParts (xs, d, e):  # -> [elems on current level], rest of xs
    ...
    if x.tag == 'part-group':
        ...
        if type == 'start': # go one level deeper
            s = [x.findtext (n, '') for n in ['group-symbol','group-barline','gr
oup-name','group-abbreviation']]
            d [num] = s     # remember groupdata by group number
            e.append (num)  # make stack of open group numbers
            elemsnext, rest1 = parseParts (xs, d, e) # parse one level deeper to
 next stop
            elems, rest2 = parseParts (rest1, d, e)  # parse the rest on this le
vel
            ...
            if xs and xs[0].get ('type') == 'stop':     # two consequetive stops
                if num != nums:                         # in the wrong order (te
mpory solution)
                    d[nums], d[num] = d[num], d[nums]   # exchange values    (on
ly works for two stops!!!)
            sym = d[num]    # retrieve an return groupdata as last element of th
e group
            return [sym], xs
    else:
        elems, rest = parseParts (xs, d, e) # parse remaining elements on curren
t level
...
def prgroupelem (x, gnm, bar, pmap, accVce, accStf):    # collect partnames (acc
Vce) and %%score map (accStf)
    if type (x) == tupletype:   # partname-tuple = (part-name, part-abbrev)
        y = pmap.pop (0)
        if gnm[0]: x = [n1 + ':' + n2 for n1, n2 in zip (gnm, x)]   # put group-
name before part-name
        accVce.append (x)
        accStf.extend (bracePart (y))
    elif len (x) == 2 and type (x[0]) == tupletype: # misuse of group just to ad
d extra name to stave
        y = pmap.pop (0)
        nms = [n1 + ':' + n2 for n1, n2 in zip (x[0], x[1][2:])]    # x[0] = par
tname-tuple, x[1][2:] = groupname-tuple
        accVce.append (nms)
        accStf.extend (bracePart (y))
    else:
        prgrouplist (x, bar, pmap, accVce, accStf)

def prgrouplist (x, pbar, pmap, accVce, accStf):    # collect partnames, scorema
p for a part-group
    sym, bar, gnm, gabbr = x[-1]    # bracket symbol, continue barline, group-na
me-tuple
    ...
    for uL in [4,8,16]:     # try 1/4, 1/8 and 1/16
        ...
        for im, m in enumerate (maten): # all measures
            for e in m[iv]: # all notes in voice iv
                if isinstance (e, Elem) or e.dur == 0: continue # no real durati
ons
                vLen += len (abcdur (e, divs [im], uL))  # add len of duration s
tring
...
def checkMelismas (lyrics, maten, im, iv):
    ...
    for n, (lyrstr, melis) in prvlyr.items ():  # all lyric numbers in the previ
ous measure
        if n not in curlyr and melis:   # melisma required, but no lyrics presen
t -> make one!
            ms = getMelisma (maat)      # get a melisma for the current measure
            if ms: curlyr [n] = (ms, 0) # set melisma as the n-th lyrics of the
current measure
...
def perc2map (abcIn):
    ...
    for x in ls:
        if 'I:percmap' in x:
            ...
            x = '%%%%map perc%s %s print=%s midi=%s heads=%s' % (id, noot, step,
 midi, kop)
            maps [id].append (x)
        if 'V:' in x:
            r = r1.match (x)
...
#----------------
class Parser:
    note_alts = [   # 3 alternative notations of the same note for tablature map
ping
        [x.strip () for x in '=C,  ^C, =D, ^D, =E, =F, ^F, =G, ^G, =A, ^A, =B'.s
plit (',')],
        [x.strip () for x in '^B,  _D,^^C, _E, _F, ^E, _G,^^F, _A,^^G, _B, _C'.s
plit (',')],
        [x.strip () for x in '__D,^^B,__E,__F,^^D,__G,^^E,__A,_/A,__B,__C,^^A'.s
plit (',')] ]
    step_map = {'C':0,'D':2,'E':4,'F':5,'G':7,'A':9,'B':11}
    def __init__ (s, options):
        # unfold repeats, number of chars per line, credit filter level, volta o
ption
        s.slurBuf = {}    # dict of open slurs keyed by slur number
        s.dirStk = {}     # {direction-type + number -> (type, voice | time)} di
ct for proper closing
        ...
        s.midDflt = [-1,-1,-1,-91] # default midi settings for channel, program,
 volume, panning
        s.msralts = {}    # xml-notenames (without octave) with accidentals from
 the key
        s.curalts = {}    # abc-notenames (with voice number) with passing accid
entals
    ...
    def matchSlur (s, type2, n, v2, note2, grace, stopgrace): # match slur numbe
r n in voice v2, add abc code to before/after
        if type2 not in ['start', 'stop']: return   # slur type continue has no
abc equivalent
        if n == None: n = '1'
        if n in s.slurBuf:
            type1, v1, note1, grace1 = s.slurBuf [n]
            if type2 != type1:              # slur complete, now check the voice
                if v2 == v1:                # begins and ends in the same voice:
 keep it
                    if type1 == 'start' and (not grace1 or not stopgrace):  # no
rmal slur: start before stop and no grace slur
                        note1.before = ['('] + note1.before # keep left-right or
der!
                        note2.after += ')'
                ...
                info ('double slur numbers %s-%s in part %d, measure %d, voice %
d note %s, first discarded' % (type2, n, s.msr.ixp+1, s.msr.ixm+1, v2, note2.ns)
)
    ...
    def doNotations (s, note, nttn, isTab):
        for key, val in s.ornaments:
            if nttn.find (key) != None: note.before += [val]  # just concat all
ornaments
        ...
        if trem != None:
            ...
            if type == 'single':
                ...
                if s.tstep:         # abc2svg version
                    if type == 'stop': note.before.insert (0, '!trem%s!' % trem.
text);
                else:               # abc2xml version
                    if type == 'start': note.before.insert (0, '!%s-!' % (int (t
rem.text) * '/'));
        fingering = nttn.findall ('technical/fingering')
        for finger in fingering:    # handle multiple finger annotations
            if not isTab: note.before += ['!%s!' % finger.text] # fingering goes
 before chord (addChord)
        ...
        if snaar != None and isTab:
            if s.tstep:
                ...
                deco = '!%s!' % snaar.text  # no double string decos (bug in mus
escore)
                if deco not in note.ntdec: note.ntdec += deco
        wvln = nttn.find ('ornaments/wavy-line')
        if wvln != None:
            if   wvln.get ('type') == 'start': note.before = ['!trill(!'] + note
.before # keep left-right order!
            elif wvln.get ('type') == 'stop': note.before = ['!trill)!'] + note.
before
        glis = nttn.find ('glissando')
        if glis == None: glis = nttn.find ('slide') # treat slide as glissando
        if glis != None:
            lt = '~' if glis.get ('line-type') =='wavy' else '-'
            if   glis.get ('type') == 'start': note.before = ['!%s(!' % lt] + no
te.before # keep left-right order!
            elif glis.get ('type') == 'stop': note.before = ['!%s)!' % lt] + not
e.before
    ...
    def tabnote (s, alt, ptc, oct, v, ntrec):
        ...
        snaar_nw, fret_nw = ntrec.tab   # the computed/annotated allocation of n
t
        for i in range (4):             # support same note on 4 strings
            na = s.note_alts [i % 3] [p]    # get alternative representation of
same note
            o = oct
            if na in ['^B', '^^B']: o -= 1  # because in adjacent octave
            if na in ['_C', '__C']: o += 1
            if '/' in na or i == 3: o = 9   # emergency notation for 4th string
case
            nt = addoct (na, o)
            snaar, fret = s.tabmap.get ((v, nt), ('', ''))  # the current alloca
tion of nt
            ...
            if i == 3:                  # new allocaion needed but none is free
                fmt = 'rejected: voice %d note %3s string %s fret %2s remains: s
tring %s fret %s'
                info (fmt % (v, nt, snaar_nw, fret_nw, snaar, fret), 1)
                ntrec.tab = (snaar, fret)
        s.tabmap [v, nt] = ntrec.tab    # for tablature map (voice, note) -> (st
ring, fret)
        return nt                       # ABC code always in key C (with midi pi
tch alterations)

    def ntAbc (s, ptc, oct, note, v, ntrec, isTab): # pitch, octave -> abc notat
ion
        acc2alt = {'double-flat':-2,'flat-flat':-2,'flat':-1,'natural':0,'sharp'
:1,'sharp-sharp':2,'double-sharp':2}
        ...
        if ntrec.tab: return s.tabnote (alt, ptc, oct, v, ntrec)    # implies s.
tstep is true (options.t was given)
...
1)
        p = addoct (ptc, oct)
        if alt == None and s.msralts.get (ptc, 0): alt = 0  # no alt but key imp
lies alt -> natural!!
        if alt == None and (p, v) in s.curalts: alt = 0     # no alt but previou
s note had one -> natural!!
        if acc == None and alt == None: return p    # no acc, no alt
            ...
            alt = int (float (alt))
            if (p, v) in s.curalts:  # the note in this voice has been altered b
efore
                if alt == s.curalts [(p, v)]: return p      # alteration still t
he same
            elif alt == s.msralts.get (ptc, 0): return p    # alteration implied
 by the key
            tieElms = note.findall ('tie') + note.findall ('notations/tied')
# in xml we have separate notated ties and playback ties
            if 'stop' in [e.get ('type') for e in tieElms]: return p    # don't
alter tied notes
            info ('accidental %d added in part %d, measure %d, voice %d note %s'
 % (alt, s.msr.ixp+1, s.msr.ixm+1, v+1, p))
        s.curalts [(p, v)] = alt
        p = ['__','_','=','^','^^'][alt+2] + p # and finally ... prepend the acc
idental
        return p
    ...
    def doNote (s, n):    # parse a musicXML note tag
        ...
        if s.isSib: v += 100 * int (n.findtext ('staff', '1'))  # repair bug in
Sibelius
...
')
        ...
        grc = n.find ('grace')
        note.grace = grc != None
        note.before, note.after = [], '' # strings with ABC stuff that goes befo
re or after a note/chord
        ...
        if r == None and n.get ('print-object') == 'no':
            if chord: return
            r = 1  # turn invisible notes (that advance the time) into invisible
 rests
        note.dur = int (dur)
        ...
        if e != None and e.text == 'none' and (not isTab or v in s.hasStems or s
.tstep):
            note.before += ['s']; abcOut.stemless = 1;
        e = n.find ('accidental')
        if e != None and e.get ('parentheses') == 'yes': note.ntdec += '!courtes
y!'
        if r != None: noot = 'x' if n.get ('print-object') == 'no' or isTab else
 'z'
        else: noot = s.ntAbc (p, int (o), n, v, note, isTab)
        if n.find ('unpitched') != None:
            clef = s.curClef [s.curStf [v]]     # the current clef for this voic
e
            step = staffStep (p, int (o), clef, s.tstep)        # (clef independ
ent) step value of note on the staff
            instr = n.find ('instrument')
            instId = instr.get ('id') if instr != None else 'dummyId'
            midi = s.drumInst.get (instId, abcMid (noot))
            nh =  n.findtext ('notehead', '').replace (' ','-') # replace spaces
 in xml notehead names for percmap
            if nh == 'x': noot = '^' + noot.replace ('^','').replace ('_','')
            if nh in ['circle-x','diamond','triangle']: noot = '_' + noot.replac
e ('^','').replace ('_','')
            if nh and n.find ('notehead').get ('filled','') == 'yes': nh += '+'
            if nh and n.find ('notehead').get ('filled','') == 'no': nh += '-'
            s.drumNotes [(v, noot)] = (step, midi, nh) # keep data for percussio
n map
        tieElms = n.findall ('tie') + n.findall ('notations/tied')  # in xml we
have separate notated ties and playback ties
        if 'start' in [e.get ('type') for e in tieElms]:            # n can have
 stop and start tie
            noot = noot + '-'
        note.beam = sum ([1 for b in n.findall('beam') if b.text in ['continue',
 'end']]) + int (note.grace)
        lyrlast = 0; rsib = re.compile (r'^.*verse')
        for e in n.findall ('lyric'):
            lyrnum = int (rsib.sub ('', e.get ('number', '1'))) # also do Sibeli
us numbers
            if lyrnum == 0: lyrnum = lyrlast + 1                # and correct Si
belius bugs
            else: lyrlast = lyrnum
        ...
        if chord: s.msc.addChord (note, noot)
            ...
            if s.curStf [v] != xmlstaff:    # the note should go to another staf
f
                dstaff = xmlstaff - s.curStf [v]    # relative new staff number
                s.curStf [v] = xmlstaff     # remember the new staff for this vo
ice
                s.msc.appendElem (v, '[I:staff %+d]' % dstaff)  # insert a move
before the note
            s.msc.appendNote (v, note, noot)
        for slur in n.findall ('notations/slur'):   # s.msc.lastnote points to t
he last real note/chord inserted above
            s.matchSlur (slur.get ('type'), slur.get ('number'), v, s.msc.lastno
te, note.grace, stopgrace) # match slur definitions
...
'F3':'bass3','G2':'treble','TAB':'tab','percussion':'perc'}
        ...
        steps = int (e.findtext ('transpose/chromatic', '0'))   # for transposin
g instrument
        fifths = e.findtext ('key/fifths')
        first = s.msc.tijd == 0 and s.msr.ixm == 0  # first attributes in first
measure
        if fifths:
            key, s.msralts = setKey (int (fifths), e.findtext ('key/mode','major
'))
            if first and not steps and abcOut.key == 'none':
                abcOut.key = key                # first measure -> header, if no
t transposing instrument or percussion part!
        ...
        s.msr.mdur = (s.msr.divs * s.msr.mtr[0] * 4) // s.msr.mtr[1]   # duratio
n of measure in xml-divisions
        for ms in e.findall('measure-style'):
            ...
            for mr in ms.findall('measure-repeat'):
                ty = mr.get('type')
                if ty == 'start':               # remember start measure number
and text voor each staff
                    s.repeat_str [n] = [s.msr.ixm, mr.text]
                    for v in voices:            # insert repeat into all voices,
 value will be overwritten at stop
                        s.msc.insertElem (v, s.repeat_str [n])
                elif ty == 'stop':              # calculate repeat measure count
 for this staff n
                    ...
                    if text_:
                        ...
                        mid_str = ""            # overwrite repeat with final st
ring
                    s.repeat_str [n][0] = '[I:repeat %s%d]' % (mid_str, repeat_c
ount)
                    del s.repeat_str [n]        # remove closed repeats
        toct = e.findtext ('transpose/octave-change', '')
        if toct: steps += 12 * int (toct)       # extra transposition of toct oc
taves
        for clef in e.findall ('clef'):         # a part can have multiple stave
s
            n = int (clef.get ('number', '1'))  # local staff number for this cl
ef
            sgn = clef.findtext ('sign')
            line = clef.findtext ('line', '') if sgn not in ['percussion','TAB']
 else ''
            cs = teken.get (sgn + line, '')
            oct = clef.findtext ('clef-octave-change', '') or '0'
            if oct: cs += {-2:'-15', -1:'-8', 1:'+8', 2:'+15'}.get (int (oct), '
')
            s.clefOct [n] = -int (oct);         # xml playback pitch -> abc nota
tion pitch
            ...
            if stfdtl and int (stfdtl.get ('number', '1')) == n:
                ...
                if strings:
                    tuning = [st.findtext ('tuning-step') + st.findtext ('tuning
-octave') for st in strings]
                    cs += ' strings=%s' % ','.join (tuning)
                capo = stfdtl.findtext ('capo')
                if capo: cs += ' capo=%s' % capo
            s.curClef [n] = cs                  # keep track of current clef (fo
r percmap)
            if first: s.clefMap [n] = cs        # clef goes to header (where it
is mapped to voices)
            else:
                voices = s.stfMap[n]            # clef change to all voices of s
taff n
                for v in voices:
                    if n != s.curStf [v]:       # voice is not at its home staff
 n
                        dstaff = n - s.curStf [v]
                        s.curStf [v] = n        # reset current staff at start o
f measure to home position
                        s.msc.appendElem (v, '[I:staff %+d]' % dstaff)
                    s.msc.appendElem (v, '[K:%s]' % cs)

    def findVoice (s, i, es):
        stfnum = int (es[i].findtext ('staff',1))   # directions belong to a sta
ff
        vs = s.stfMap [stfnum]                      # voices in this staff
        v1 = vs [0] if vs else 1                    # directions to first voice
of staff
        if s.dirtov1: return stfnum, v1, v1         # option --v1
        for e in es [i+1:]:                         # or to the voice of the nex
t note
            if e.tag == 'note':
                v = int (e.findtext ('voice', '1'))
                if s.isSib: v += 100 * int (e.findtext ('staff', '1'))  # repair
 bug in Sibelius
                stf = s.vce2stf [v]                 # use our own staff allocati
on
                return stf, v, v1                   # voice of next note, first
voice of staff
            if e.tag == 'backup': break
        return stfnum, v1, v1                       # no note found, fall back t
o v1

    def doDirection (s, e, i, es): # parse a musicXML direction tag
        def addDirection (x, vs, tijd, stfnum):
            if not x: return
            vs = s.stfMap [stfnum] if '!8v' in x else [vs]  # ottava's go to all
 voices of staff
            for v in vs:
                if tijd != None:   # insert at time of encounter
                    s.msc.appendElemT (v, x.replace ('(',')').replace ('ped','pe
d-up'), tijd)
                else:
                    s.msc.appendElem (v, x)
        def startStop (dtype, vs, stfnum=1):
            typmap = {'down':'!8va(!', 'up':'!8vb(!', 'crescendo':'!<(!', 'dimin
uendo':'!>(!', 'start':'!ped!'}
            type = t.get ('type', '')
            k = dtype + t.get ('number', '1')       # key to match the closing d
irection
            if type in typmap:                      # opening the direction
                x = typmap [type]
                if k in s.dirStk:                   # closing direction already
encountered
                    stype, tijd = s.dirStk [k]; del s.dirStk [k]
                    if stype == 'stop':
                        addDirection (x, vs, tijd, stfnum)
                    else:
                        info ('%s direction %s has no stop in part %d, measure %
d, voice %d' % (dtype, stype, s.msr.ixp+1, s.msr.ixm+1, vs+1))
                        s.dirStk [k] = ((type , vs))    # remember voice and typ
e for closing
                else:
                    s.dirStk [k] = ((type , vs))    # remember voice and type fo
r closing
            elif type == 'stop':
                if k in s.dirStk:                   # matching open direction fo
und
                    type, vs = s.dirStk [k]; del s.dirStk [k]   # into the same
voice
                    if type == 'stop':
                        info ('%s direction %s has double stop in part %d, measu
re %d, voice %d' % (dtype, type, s.msr.ixp+1, s.msr.ixm+1, vs+1))
                        x = ''
                    else:
                        x = typmap [type].replace ('(',')').replace ('ped','ped-
up')
                else:                               # closing direction found be
fore opening
                    s.dirStk [k] = ('stop', s.msc.tijd)
                    x = ''                          # delay code generation unti
l opening found
        ...
        jmps = [('dacapo','D.C.'),('dalsegno','D.S.'),('tocoda','dacoda'),('fine
','fine'),('coda','O'),('segno','S')]
        t = e.find ('sound')        # there are many possible attributes for sou
nd
        if t != None:
            ...
            if minst:
                ...
                vids = [v for v, id in s.vceInst.items () if id == minst.get ('i
d')]
                if vids: vs = vids [0]          # direction for the indentified
voice, not the staff
                parm, inst = ('program', str (int (prg) - 1)) if prg else ('chan
nel', chn)
                if inst and abcOut.volpan > 0: s.msc.appendElem (vs, '[I:MIDI= %
s %s]' % (parm, inst))
            tempo = t.get ('tempo') # look for tempo attribute
            if tempo:
                tempo = '%.0f' % float (tempo) # hope it is a number and insert
in voice 1
        ...
        for dirtyp in dirtypes:
            units = { 'whole': (1,1), 'half': (1,2), 'quarter': (1,4), 'eighth':
 (1,8) }
            ...
            if metr != None:
                ...
                if metr.find ('beat-unit-dot') != None:
                    tempo_units = simplify (tempo_units [0] * 3, tempo_units [1]
 * 2)
                tmpro = re.search ('[.\d]+', metr.findtext ('per-minute'))  # lo
ok for a number
                if tmpro: tempo = tmpro.group () # overwrites the value set by t
he sound element of this direction
            t = dirtyp.find ('wedge')
            if t != None: startStop ('wedge', vs)
            allwrds = dirtyp.findall ('words')        # insert text annotations
            if not allwrds: allwrds = dirtyp.findall ('rehearsal')   # treat reh
earsal mark as text annotation
            for wrds in allwrds:
                if jmp: # ignore the words when a jump sound element is present
in this direction
...
'\\n')
            ...
            if t != None: startStop ('octave-shift', vs, stf)   # assume size ==
 8 for the time being
            t = dirtyp.find ('pedal')
            if t != None and s.ped:
                if not s.pedVce: s.pedVce = vs
                startStop ('pedal', s.pedVce)
            if dirtyp.findtext ('other-direction') == 'diatonic fretting': s.dia
fret = 1;
        if tempo:
            tempo = '%.0f' % float (tempo) # hope it is a number and insert in v
oice 1
            if s.msc.tijd == 0 and s.msr.ixm == 0:      # first measure -> heade
r
                abcOut.tempo = tempo
                abcOut.tempo_units = tempo_units
            else:
                s.msc.appendElem (v1, '[Q:%d/%d=%s]' % (tempo_units [0], tempo_u
nits [1], tempo))   # otherwise -> 1st voice
        if wrdstxt: s.msc.appendElem (vs, '"%s%s"' % (plc, wrdstxt), 1) # to voi
ce, but after tempo

    def doHarmony (s, e, i, es):    # parse a musicXMl harmony tag
        _, vt, _ = s.findVoice (i, es)
        short = {'major':'', 'minor':'m', 'augmented':'+', 'diminished':'dim', '
dominant':'7', 'half-diminished':'m7b5'}
        accmap = {'major':'maj', 'dominant':'', 'minor':'m', 'diminished':'dim',
 'augmented':'+', 'suspended':'sus'}
        modmap = {'second':'2', 'fourth':'4', 'seventh':'7', 'sixth':'6', 'ninth
':'9', '11th':'11', '13th':'13'}
        altmap = {'1':'#', '0':'', '-1':'b'}
        ...
        if kind in short: kind = short [kind]
            ...
            if kind.startswith ('sus'): kind, sus = '', kind    # sus-suffix goe
s to the end
        elif kind == 'none': kind = e.find ('kind').get ('text','')
        degrees = e.findall ('degree')
        for d in degrees:   # chord alterations
            kind += altmap.get (d.findtext ('degree-alter'),'') + d.findtext ('d
egree-value','')
        kind = kind.replace ('79','9').replace ('713','13').replace ('maj6','6')
        bass = e.findtext ('bass/bass-step','') + altmap.get (e.findtext ('bass/
bass-alter'),'')
        s.msc.appendElem (vt, '"%s%s%s%s%s"' % (root, alt, kind, sus, bass and '
/' + bass), 1)

    def doBarline (s, e):       # 0 = no repeat, 1 = begin repeat, 2 = end repea
t
        rep = e.find ('repeat')
        ...
        if end != None:
            if end.get ('type') == 'start':
                ...
                try: list (map (int, n.split (',')))    # should be a list of in
tegers
                except: n = '"%s"' % n.strip ()         # illegal musicXML
                s.msr.lnum = n          # assume a start is always at the beginn
ing of a measure
            elif s.msr.rline == '|':    # stop and discontinue the same  in ABC
?
                s.msr.rline = '||'      # to stop on a normal barline use || in
ABC ?
        return 0

    def doPrint (s, e):     # print element, measure number -> insert a line bre
ak
        if e.get ('new-system') == 'yes' or e.get ('new-page') == 'yes':
            if not s.nolbrk: return '$'  # a line break

    def doPartList (s, e):  # translate the start/stop-event-based xml-partlist
into proper tree
        for sp in e.findall ('part-list/score-part'):
            midi = {}
            for m in sp.findall ('midi-instrument'):
                x = [m.findtext (p, s.midDflt [i]) for i,p in enumerate (['midi-
channel','midi-program','volume','pan'])]
                pan = float (x[3])
                if pan >= -90 and pan <= 90:    # would be better to map behind-
pannings
                    pan = (float (x[3]) + 90) / 180 * 127   # xml between -90 an
d +90
                midi [m.get ('id')] = [int (x[0]), int (x[1]), float (x[2]) * 1.
27, pan]    # volume 100 -> midi 127
                up = m.findtext ('midi-unpitched')
                if up: s.drumInst [m.get ('id')] = int (up) - 1 # store midi-pit
ch for channel 10 notes
            s.instMid.append (midi)
        ps = e.find ('part-list')               # partlist  = [groupelem]
        xs = getPartlist (ps)                   # groupelem = partname | groupli
st
        partlist, _ = parseParts (xs, {}, [])   # grouplist = [groupelem, ..., g
roupdata]
        return partlist                         # groupdata = [group-symbol, gro
up-barline, group-name, group-abbrev]

    def mkTitle (s, e):
        def filterCredits (y):  # y == filter level, higher filters less
            cs = []
            for x in credits:   # skip redundant credit lines
                if y < 6 and (x in title or x in mvttl): continue         # sure
 skip
                if y < 5 and (x in composer or x in lyricist): continue   # almo
st sure skip
                if y < 4 and ((title and title in x) or (mvttl and mvttl in x)):
 continue   # may skip too much
                if y < 3 and ([1 for c in composer if c in x] or [1 for c in lyr
icist if c in x]): continue # skips too much
                if y < 2 and re.match (r'^[\d\W]*$', x): continue       # line o
nly contains numbers and punctuation
                cs.append (x)
            if y == 0 and (title + mvttl): cs = ''  # default: only credit when
no title set
            return cs
        ...
        s.isSib = 'Sibelius' in (e.findtext ('identification/encoding/software')
 or '')
    ...
    def doDefaults (s, e):
        ...
        abcScale = space / 0.2117   # 0.2117 cm = 6pt = space between staff line
s for scale = 1.0 in abcm2ps
        abcOut.pageFmt ['scale'] = abcScale
        eks = 2 * ['page-layout/'] + 4 * ['page-layout/page-margins/']
        eks = [a+b for a,b in zip (eks, 'page-height,page-width,left-margin,righ
t-margin,top-margin,bottom-margin'.split (','))]
        for i in range (6):
            v = d.findtext (eks [i])
            k = abcOut.pagekeys [i+1]   # pagekeys [0] == scale already done, sk
ip it
            if not abcOut.pageFmt [k] and v:
                try: abcOut.pageFmt [k] = float (v) * xmlScale  # -> cm
                except: info ('illegal value %s for xml element %s', (v, eks [i]
)); continue    # just skip illegal values

    def locStaffMap (s, part, maten):   # map voice to staff with majority votin
g
        vmap = {}   # {voice -> {staff -> n}} count occurrences of voice in staf
f
        s.vceInst = {}          # {voice -> instrument id} for this part
        s.msc.vnums = {}        # voice id's
        s.hasStems = {}         # XML voice nums with at least one note with a s
tem (for tab key)
        s.stfMap, s.clefMap = {}, {}    # staff -> [voices], staff -> clef
        ns = part.findall ('measure/note')
        for n in ns:            # count staff allocations for all notes
            v = int (n.findtext ('voice', '1'))
            if s.isSib: v += 100 * int (n.findtext ('staff', '1'))  # repair bug
 in Sibelius
            ...
            if v not in vmap:
                ...
                d[sn] = d.get (sn, 0) + 1   # ++ number of allocations for staff
 sn
            x = n.find ('instrument')
            if x != None: s.vceInst [v] = x.get ('id')
            x, noRest = n.findtext ('stem'), n.find ('rest') == None
            if noRest and (not x or x != 'none'): s.hasStems [v] = 1    # XML vo
ice v has at least one stem
        vks = list (vmap.keys ())
        if s.jscript or s.isSib: vks.sort ()
        for v in vks:           # choose staff with most allocations for each vo
ice
    ...
    def addStaffMap (s, vvmap): # vvmap: xml voice number -> global abc voice nu
mber
        part = [] # default: brace on staffs of one part
        for stf, voices in sorted (s.stfMap.items ()):  # s.stfMap has xml staff
 and voice numbers
...
# same order as locmap
            if locmap:          # abc voice number of staff stf
                part.append (locmap)
                clef = s.clefMap.get (stf, 'treble')    # {xml staff number -> c
lef}
                for i, iv in enumerate (locmap):
                    clef_attr = ''
                    if clef.startswith ('tab'):
                        if nostem [i] and 'nostems' not in clef: clef_attr = ' n
ostems'
                        if s.diafret and 'diafret' not in clef: clef_attr += ' d
iafret' # for all voices in the part
                    abcOut.clefs [iv] = clef + clef_attr # add nostems when all
notes of voice had no stem
    ...
    def addMidiMap (s, ip, vvmap):      # map abc voices to midi settings
        ...
        if instr.values (): defInstr = list(instr.values ())[0]   # default sett
ings = first instrument
...
if v == vd] # map perc notes
            id = s.vceInst.get (v, '')  # get the instrument-id for part with mu
ltiple instruments
            if id in instr:             # id is defined as midi-instrument in pa
rt-list
                   xs.append ((vabc, instr [id] + ds))  # get midi settings for
id
            else:  xs.append ((vabc, defInstr   + ds))  # only one instrument fo
r this part
        xs.sort ()  # put abc voices in order
        s.midiMap.extend ([midi for v, midi in xs])
        snaarmap = ['E','G','B','d', 'f', 'a', "c'", "e'"]
        diamap = '0,1-,1,1+,2,3,3,4,4,5,6,6+,7,8-,8,8+,9,10,10,11,11,12,13,13+,1
4'.split (',')
        ...
        for ip, p in enumerate (parts):
            ...
            while s.msr.ixm < len (maten):
                ...
                for i, e in enumerate (es):
                    ...
                    elif e.tag == 'sound':      s.doDirection (maat, i, es) # so
und element directly in measure!
            ...
            for rv in s.repeat_str.values ():   # close hanging measure-repeats
without stop
